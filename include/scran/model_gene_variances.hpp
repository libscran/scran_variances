...
#include "tatami/tatami_stats.hpp"
...
struct Options {
     ...
     * Only used when `Options::block_weight_policy` is set to `block_weights::Policy::VARIABLE`.
     */
     ...
     * Note that this only affects the `compute_blocked()` method that returns a `BlockedResults` object.
     */
